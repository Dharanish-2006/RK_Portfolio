...
transporter.verify(err => {
  if (err) {
    console.error('⚠️  Mailer NOT connected:', err.message);
    console.error('   → Check GMAIL_USER and GMAIL_APP_PASSWORD in server/.env');
  } else {
    console.log('✅  Nodemailer connected — Gmail SMTP ready');
  }
});

function buildAdminEmail({ name, email, company, message, phone }) {
  const co  = company || '—';
  const ph  = phone   || '—';
...
<style>
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .badge{display:inline-block;background:rgba(233,193,118,.18);
         border:1px solid rgba(233,193,118,.35);border-radius:100px;
         padding:4px 14px;font-size:11px;font-weight:700;
         letter-spacing:.14em;color:#e9c176;text-transform:uppercase;margin-bottom:14px}
  .hd h1{font-size:26px;font-weight:700;color:#fff;line-height:1.3}

  .bd{background:#fff;padding:40px 44px}
  .eyebrow{display:flex;align-items:center;gap:8px;
           font-size:10px;font-weight:700;letter-spacing:.2em;
           color:#0059bb;text-transform:uppercase;margin-bottom:18px}
  .eyebrow::before{content:'';display:inline-block;width:20px;height:2px;
                   background:#0059bb;border-radius:1px}
  ...
  .fl{font-size:10px;font-weight:700;letter-spacing:.12em;
  ...
  .msg-box{background:#f7f9fb;border:1px solid #e2e8f0;
           border-left:4px solid #0059bb;border-radius:0 11px 11px 0;
  ...
  .cta-btn{display:inline-block;background:#0059bb;color:#fff;
           font-size:14px;font-weight:700;padding:13px 30px;
           border-radius:100px;letter-spacing:.03em}
  .cta-btn:hover{background:#003f87}
...
<div class="shell">
      ...
      <div class="logo-box">
        <svg width="19" height="19" viewBox="0 0 24 24" fill="none">
          ...
          <path d="M2 12h20M12 2a15 15 0 0 1 4 10 15 15 0 0 1-4 10 15 15 0 0 1-4-10 15 15 0 0 1 4-10z"
                stroke="rgba(233,193,118,.9)" stroke-width="1.5" fill="none"/>
        </svg>
      </div>
  ...
  <div class="bd">
    ...
    <div class="grid">
      <div class="field">
        ...
        <div class="fv">${esc(name)}</div>
      </div>
      <div class="field">
        <div class="fl">Email Address</div>
        <div class="fv"><a href="mailto:${esc(email)}">${esc(email)}</a></div>
      </div>
      <div class="field">
        <div class="fl">Company</div>
        <div class="fv">${esc(co)}</div>
      </div>
      <div class="field">
        <div class="fl">Phone</div>
        <div class="fv">${esc(ph)}</div>
      </div>
    </div>
  ...
  <div class="ft">
    <p>RK Exports &amp; Imports · Global Trade Authority<br/>
...
app.get('/api/health', (_req, res) => {
  res.json({ ok: true, service: 'RK Exports Mailer', ts: new Date().toISOString() });
});
...
app.post('/api/contact', async (req, res) => {
  ...
  try {
    await transporter.sendMail({
      ...
      html   : buildAdminEmail(clean),
    });

    console.log(`📧  Admin notified — from ${clean.email}  [${new Date().toISOString()}]`);
    return res.json({ ok: true, message: `Your enquiry has been sent. We'll be in touch soon!` });

  } catch (err) {
    console.error('❌  Send error:', err.message);
    return res.status(500).json({ ok: false, message: 'Failed to send. Please try again or email us directly.' });
  }
});
...
app.listen(PORT, () => {
  console.log(`\n🌐  RK Exports Mailer  →  http://localhost:${PORT}/api/contact\n`);
});